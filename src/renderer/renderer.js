document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── Test framework switching (Cypress vs Playwright) ─────── */
  let activeFramework = 'cypress';

  document.querySelectorAll('#frameworkTabs .provider-tab').forEach(btn => {
    btn.addEventListener('click', () => switchFramework(btn.dataset.framework));
  });

  function switchFramework(fw) {
    activeFramework = fw;
    document.querySelectorAll('#frameworkTabs .provider-tab').forEach(t => {
      t.classList.toggle('active', t.dataset.framework === fw);
    });
    console.log('Framework switched to:', fw);
  }

  // ── Per-framework config: file extensions, naming, prompt vocabulary ──
  const FRAMEWORK_CONFIG = {
    cypress: {
      label:        'Cypress',
      specExt:      '.cy.js',
      specDir:      (m) => `cypress/e2e/${m}`,
      pageDir:      (m) => `cypress/pages/${m}`,
      fixtureDir:   () => `cypress/fixtures`,
      supportDir:   () => `cypress/support`,
      utilsDir:     () => `cypress/utils`,
      reportsDir:   () => `cypress/reports`,
      engineerNoun: '${fw().label} automation engineer',
      architectNoun:'Output Cypress 13 files',
      configFiles:  [
        'package.json — include cypress@13, @faker-js/faker, cypress-axe, dotenv',
        'cypress.config.js — baseUrl set to {baseUrl}, retries:2, video:false, specPattern: cypress/e2e/**/*.cy.js, reporter: json with output to cypress/reports/results.json',
        'cypress/support/e2e.js — global hooks, import commands, axe setup',
        'cypress/support/commands.js — cy.login(), cy.apiRequest(), cy.healLocator()',
        'cypress/utils/selfHeal.js — healing chain: data-cy > aria-label > text > CSS > positional, logs which succeeded',
      ],
      runHint: 'npx cypress open',
    },
    playwright: {
      label:        'Playwright',
      specExt:      '.spec.js',
      ...
      engineerNoun: 'Playwright automation engineer',
      architectNoun:'Output Playwright Test (@playwright/test) v1 files',
      configFiles:  [
        'package.json — include @playwright/test, @faker-js/faker, dotenv',
        'playwright.config.js — baseURL set to {baseUrl}, retries:2, video:"off", testDir: "tests", reporter: [["json",{outputFile:"playwright-report/results.json"}]]',
        'support/globalSetup.js — login/auth state setup, saved to storageState.json',
        'support/commands.js — exported helper functions: login(page), apiRequest(request), healLocator(page, candidates)',
        'utils/selfHeal.js — healing chain: data-testid > aria-label > text > CSS > positional, logs which succeeded',
      ],
      runHint: 'npx playwright test --ui',
    },
  };

  function fw() { return FRAMEWORK_CONFIG[activeFramework]; }
...
/* ── Claude model selector ────────────────────────────────── */
  ...
  saveBtn?.addEventListener('click', async () => {
    const text = results[activeTab];
    if (!text) return;
    const names = activeFramework === 'playwright'
      ? { 0:'test-cases.md', 1:'playwright-framework.md', 2:'test-scripts.js', 3:'self-heal-report.md' }
      : { 0:'test-cases.md', 1:'cypress-framework.md', 2:'test-scripts.js', 3:'self-heal-report.md' };
...
if (results[2]) addReqWrap?.removeAttribute('hidden');
  ...
  async function saveFrameworkToDisk() {
    ...
    if (results[3]) allFiles.push({ name: `${fw().reportsDir()}/self-heal-report.md`, content: results[3] });
    if (results[0]) allFiles.push({ name: `${fw().reportsDir()}/test-cases.md`,       content: results[0] });
...
- Generate maximum 100 test cases per module, but fewer is fine if requirements are limited`,
    ...
    const camel = moduleName.replace(/\s+/g,'');
    const configFiles = await callClaude(apiKey,
      `You are a test automation architect. ${fw().architectNoun} for ONE module.
...
===ENDFILE===`,
      ...
      `1. ${fw().pageDir(camel)}/index.js  (page object)\n` +
      `2. ${fw().fixtureDir()}/${slug}.json  (test data)\n` +
      `3. ${fw().supportDir()}/${slug}-commands.js  (custom commands/helpers)`
    );
    ...
    const scripts = await callClaude(apiKey,
      `You are a ${fw().engineerNoun}. Write runnable ${fw().label} spec files for ONE module.
CRITICAL: Use ONLY this exact format — no other text outside markers:
===FILE: path/filename===
file content here
===ENDFILE===`,
      ...
      `1. ${fw().specDir(slug)}/happy-path${fw().specExt}\n` +
      `2. ${fw().specDir(slug)}/edge-cases${fw().specExt}`
    );
    ...
    const healReport = await callClaude(apiKey,
      `You are a test resilience engineer. Analyse ${fw().label} specs and produce a self-healing report.
IMPORTANT: Only flag a selector if it is genuinely broken or highly likely to break (e.g. auto-generated IDs, nth-child indexes, volatile CSS class names, deeply nested positional chains).
Do NOT flag a selector simply because a different selector type is preferred. A stable ID, a meaningful class name, or any selector that reliably identifies the element is perfectly fine — leave it alone.
For every genuinely broken/fragile selector output:
STALE: <original>
REASON: specifically why this will break (not just "not preferred")
✓ HEALED: <better selector>
STRATEGY: data-cy/data-testid | aria | text | composite`,
      `Module: ${moduleName}\n\nSpecs:\n${scripts.substring(0, 1400)}\n\n` +
      `Flag fragile selectors. Propose healing chain appropriate for ${fw().label}.`
    );
    ...
    if (folderPath && window.electronAPI?.writeFiles) {
      ...
      allFiles.push({
        name: `${fw().reportsDir()}/test-cases-${moduleName.toLowerCase().replace(/\s+/g,'-')}.md`,
...
const RESULTS_PATH = process.env.RESULTS_PATH
  ? path.join(__dirname, '..', process.env.RESULTS_PATH)
  : (fs.existsSync(path.join(__dirname, '../playwright-report/results.json'))
      ? path.join(__dirname, '../playwright-report/results.json')
      : path.join(__dirname, '../cypress/reports/results.json'));
...
function walkPlaywright(suites) {
      ...
      (spec.tests || []).forEach(t => {
        (t.results || []).forEach(r => {
          if (r.status === 'failed' || r.status === 'timedOut') {
            failures.push({
              suite: spec.file || suite.title || 'Unknown Suite',
              title: spec.title || t.title || 'Unknown Test',
              error: (r.error && (r.error.message || r.error.stack)) || 'No error message'
            });
          }
        });
      });
    });
    if (suite.suites) walkPlaywright(suite.suites); // nested suites
  });
}

if (Array.isArray(results.suites)) {
  // Playwright JSON reporter shape
  walkPlaywright(results.suites);
} else {
  // Cypress/Mocha shape
  (results.results || []).forEach(suite => {
    const specs = suite.tests || [];
    specs.forEach(test => {
...
`;
    ...
    const githubWorkflow = (activeFramework === 'playwright') ? `name: Playwright + AI Failure Analysis
  ...
  playwright-and-analyze:
    ...
    steps:
      ...
      - name: Upload reports as artifacts
        ...
        with:
          name: playwright-ai-reports
          path: |
            playwright-report/results.json
            playwright-report/failure-analysis.md
            playwright-report/
            test-results/
          retention-days: 14
      ...
      - name: Comment analysis on PR
        ...
        with:
          script: |
            const fs = require('fs');
            const reportPath = 'playwright-report/failure-analysis.md';
...
` : `name: Cypress + AI Failure Analysis
...
`;
  ...
  async function generateSharedFiles(apiKey, baseUrl, folderPath) {
    ...
    const sharedConfig = await callClaude(apiKey,
      `You are a test automation architect. ${fw().architectNoun}.
CRITICAL: Use ONLY this exact format — no other text outside markers:
===FILE: path/filename===
file content here
===ENDFILE===`,
      ...
      fw().configFiles.map((f, i) => `${i+1}. ${f.replace('{baseUrl}', baseUrl)}`).join('\n') +
      `\n${fw().configFiles.length + 1}. .env.example — BASE_URL, USERNAME, PASSWORD, API_KEY, ANTHROPIC_API_KEY vars\n` +
      `${fw().configFiles.length + 2}. README.md — setup steps: npm install, ${fw().runHint}, env setup, folder structure, and a section explaining AI Failure Analyzer usage`
    );
  ...
  async function sendChatMessage() {
    ...
    showChatTyping();
    const system =
      `You are an expert ${fw().label} automation engineer fixing bugs in a generated test framework.\n\n` +
      `When fixing bugs:\n` +
  ...
  tcSelectConfirm?.addEventListener('click', async () => {
    ...
    try {
      ...
      const scripts1 = await callClaude(apiKey,
        `You are a ${fw().label} automation engineer. Write runnable ${fw().label} spec files ONLY for the provided test cases.
CRITICAL: Use ONLY this format:
===FILE: path/filename===
file content here
===ENDFILE===`,
        `Base URL: ${$('baseUrl')?.value.trim()}\n\n` +
        `Generate ${fw().label} spec files for ONLY these ${selectedTCs.length} test cases. Use file extension ${fw().specExt}:\n\n` +
...
===ENDFILE===`,
        `Base URL: ${$('baseUrl')?.value.trim()}\n\n` +
        `Continue generating ${fw().label} specs for remaining test cases. Use file extension ${fw().specExt}:\n\n` +
